...
<a href="https://wa.me/5215669989384" target="_blank" rel="noopener noreferrer" class="whatsapp-float"
...
function creatblue_pdf_gate_asset_uri($relative_path)
{
    $relative_path = ltrim($relative_path, '/');

    $style_file = trailingslashit(get_stylesheet_directory()) . $relative_path;
    if (file_exists($style_file)) {
        return trailingslashit(get_stylesheet_directory_uri()) . $relative_path;
    }

    $template_file = trailingslashit(get_template_directory()) . $relative_path;
...
function creatblue_pdf_gate_emergency_loader()
{
    if (is_admin()) {
        return;
    }

    $css_rel = 'pdf-leads-gate/assets/css/pdf-leads-gate.css';
    $js_rel = 'pdf-leads-gate/assets/js/pdf-leads-gate.js';
    $css_url = creatblue_pdf_gate_asset_uri($css_rel);
    $js_url = creatblue_pdf_gate_asset_uri($js_rel);

    if (empty($js_url)) {
        return;
    }

    $js_ver_file = trailingslashit(get_stylesheet_directory()) . $js_rel;
    if (!file_exists($js_ver_file)) {
        $js_ver_file = trailingslashit(get_template_directory()) . $js_rel;
    }
    $js_ver = file_exists($js_ver_file) ? (string) filemtime($js_ver_file) : '1.0.0';

    $css_ver_file = trailingslashit(get_stylesheet_directory()) . $css_rel;
    if (!file_exists($css_ver_file)) {
        $css_ver_file = trailingslashit(get_template_directory()) . $css_rel;
    }
    $css_ver = file_exists($css_ver_file) ? (string) filemtime($css_ver_file) : '1.0.0';
    ?>
    <script>
        window.PLG_PDF_LEADS = window.PLG_PDF_LEADS || {
            ajax_url: '<?php echo esc_url(admin_url('admin-ajax.php')); ?>',
            nonce: '<?php echo esc_js(wp_create_nonce('plg_pdf_leads_nonce')); ?>',
            debug: true,
            messages: {
        ...
        console.log('[PDF GATE EMERGENCY] loader initialized', {
            hasPrimaryStateFn: typeof window.PLG_DEBUG_STATE === 'function',
            ajaxUrl: window.PLG_PDF_LEADS.ajax_url,
            hasNonce: !!window.PLG_PDF_LEADS.nonce
        });
    </script>
    <?php if (!empty($css_url)) : ?>
        <link rel="stylesheet" href="<?php echo esc_url(add_query_arg('ver', $css_ver, $css_url)); ?>">
    <?php endif; ?>
    <script src="<?php echo esc_url(add_query_arg('ver', $js_ver, $js_url)); ?>"></script>
    <?php
}
add_action('wp_footer', 'creatblue_pdf_gate_emergency_loader', 9999);
